...
public partial class Chunk : MonoBehaviour
{
    ...
    public class ChunkData // Maybe do nested data classes for all serializables?
    {
        ...
        public ChunkData(Chunk chunk)
        {
            ...
            // Filter out null/destroyed WorldObjects before saving
            List<WorldObject> validWorldItems = new List<WorldObject>();
            foreach (WorldObject wi in chunk.worldItems)
            {
                if (wi != null && wi.gameObject != null)
                {
                    validWorldItems.Add(wi);
                }
            }
            
            worldItemData = new WorldObjectData[validWorldItems.Count];
            int i = 0;
            foreach (WorldObject wi in validWorldItems)
            {
                try
                {
                ...
                {
                    Debug.LogError($"Error saving WorldObject {wi.name}: {e.Message}");
                }
            }
        }
    }
